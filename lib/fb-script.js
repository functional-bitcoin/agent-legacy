...
class FBScript {
  ...
  loadScript(force) {
    return Promise.resolve(!!force ? null : config.cache.getScript(this.txid))
      .then(res => {
        if (res) return res;
  ...
  loadFunctions(force) {
    ...
    return Promise.resolve(!!force ? null : config.cache.getFunctions(cmds))
      .then(res => {
  ...
  execute(force) {
    return Promise.resolve(!!force ? null : config.cache.getResult(this.txid))
      .then(res => {
        if (res) return res;
        let ctx;
        ...
        .then(res => {
          config.cache.setResult(this.txid, res)
          return res;
        })
      })
      .then(res => {
