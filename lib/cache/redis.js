...
const redisCache = {
  ...
  setScript(txid, tx) {
    const key = this.buildKey('S', txid),
          ttl = tx.blk ? this.ttlConfirmed : this.ttlUnconfirmed,
  ...
  setFunctions(args) {
    ...
    args.forEach(a => {
      ...
      multi.setex(key, this.ttlConfirmed, payload)
    })
    return multi.exec()
  },

  getResult(txid) {
    const key = this.buildKey('R', txid);
    return this.getAsync(key)
      .then(res => {
        if (res) return JSON.parse(res);
      })
  ...
  setResult(txid, res) {
    ...
    return this.client.setex(key, this.ttlConfirmed, payload)
  },
